...
ORANGE2 = (251, 146, 60)
WHITE   = (255, 255, 255)
LIGHT   = (209, 213, 219)   # #D1D5DB — readable subtext
GRAY    = (107, 114, 128)
MUTED   = (75,  85,  99)
DARK1   = (17,  24,  39)
GREEN   = (78,  201, 176)
...
def fit_text(text, max_size, min_size, max_width, d, bold=False):
    """Return (font, wrapped_lines) auto-scaled to fit max_width."""
    size = max_size
    while size >= min_size:
        ...
        for word in words:
            ...
            if bbox[2]-bbox[0] <= max_width:
                ...
                current = [word]
        if current: lines.append(" ".join(current))
        # Check total height — assume 1.3em line height
        total_h = len(lines) * int(size * 1.3)
        if total_h <= max_size * 6:   # generous height budget
            return f, lines
        size -= 4
    return get_font(min_size, bold), lines

def draw_centred_lines(d, lines, font, start_y, line_h, colours=None, W=1080, pad=80):
    """Draw lines centred horizontally, return final y."""
    y = start_y
...
def render_tip_repurpose(data):
    ...
    # Main tip — auto-scaled, bold white
...
def render_poll(data):
    ...
    for i, opt in enumerate(options[:2]):
        bg  = ORANGE if i == 0 else BG2
        fg  = BG     if i == 0 else WHITE
        if i == 1:
            outline_pill(d, W//2, y, opt, f_opt, pad_x=120, h=110)
        else:
            pill(d, W//2, y, opt, f_opt, bg=bg, fg=fg, pad_x=120, h=110)
        y += 130

    # Sticker reminder — dashed box
    y += 20
    d.rounded_rectangle([120, y, W-120, y+80],
                         radius=16, outline=ORANGE, width=3)
    # Dashed effect — draw over with BG segments
...
def render_weekly_roundup(data, slide_num=1, total_slides=3):
    ...
    if slide_num == 1:
        ...
        d.text((W//2, 210), "Best moment this week",
               font=get_font(54, True), fill=WHITE, anchor="mm")

        stat_num  = data.get("stat_number", "")
        stat_desc = data.get("stat_description", "")

        f_big = get_font(200, True)
        bbox  = d.textbbox((0,0), stat_num, font=f_big)
        tw    = bbox[2]-bbox[0]
        d.text(((W-tw)//2, 360), stat_num, font=f_big, fill=ORANGE)

        f_d, d_lines = fit_text(stat_desc, 52, 38, W-160, d)
        d_h = int(f_d.size * 1.4)
        draw_centred_lines(d, d_lines, f_d, 760, d_h,
                           colours=[LIGHT]*10, W=W)
...
def generate_story(story_type=None):
    ...
    if story_type == "tip_repurpose":
        ...
        # Skip slide 2 (stats) until real Instagram insights are available
        # Once fetch_insights.py is built, re-enable slide 2 with real data
        slides_to_render = [
            (1, data.get("slide1", {})),
            (3, data.get("slide3", {})),
        ]
