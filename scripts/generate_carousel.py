...
def draw_logo(d, x, y):
    """Draw mini logo mark at given position."""
    bw, bh = 100, 74
...
def render_cover_slide(slide, post_id, slide_num, total, topic):
    ...
    d.text((S-48, 72), f"1 / {total}",
           font=get_mono(20), fill=GRAY, anchor="rm")

    # Main title — large, centred
...
def render_content_slide(slide, post_id, slide_num, total):
    """Slides 2 to N-1 — tip content slides."""
    S = 1080
    img = Image.new("RGB", (S, S), BG)
    d   = ImageDraw.Draw(img)
...
def render_cta_slide(slide, post_id, total, handle="@ask.claudeai"):
    ...
    # Handle
    d.text((S//2, 940), handle,
           font=get_mono(28), fill=GRAY, anchor="mm")
...
def render_slides(slides_data, post_id, topic):
    """Render all slides and save to queue/images/post_id/."""
    ...
    for i, slide in enumerate(slides_data):
        ...
        images.append(path)
        print(f"  Slide {i+1}/{total} saved: {path}")
...
- Max 2 lines of body text per content slide (keep it scannable)
...
Include exactly {n_slides} slides total (1 cover + {n_slides-2} content + 1 CTA)."""
    ...
    print(f"\nOpen slides to preview:")
    print(f"  open queue/images/{post_id}/")
